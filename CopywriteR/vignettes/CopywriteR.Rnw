...
advantages relative to current approaches \cite{Kuilman:vf} and it constitutes a
...
in a new folder and are required to run CopywriteR. If previously created helper
files for a specific bin size / reference genome combination are available, the
...
argument can be either "mm9", "mm10" or "hg19". If you are interested in getting
helper files for other genomes, please contact me by the email addresses
supplied at the beginning of this vignette.
