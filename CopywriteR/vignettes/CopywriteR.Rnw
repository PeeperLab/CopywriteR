...
argument can be either "hg18", "hg19", "hg38", "mm9" or "mm10". The default
chromosome notation is "1", "2", ..., "X", "Y". The prefix argument is optional
and can be used to add a prefix to these names.
...
<<CopywriteR, print=FALSE, eval=FALSE>>=
...
CopywriteR(sample.control = sample.control,
           ...
           bp.param = bp.param)
@
<<CopywriteR, print=FALSE, eval=TRUE, results=hide, echo=FALSE>>=
file.copy(system.file(package = "CopywriteR", "extdata", "CNAprofiles"),
          data.folder, recursive = TRUE, copy.mode = TRUE)
@
...
read.table(file = file.path(data.folder, "CNAprofiles",
                            "log2_read_counts.igv"), header = TRUE)[133:138, ]
@
