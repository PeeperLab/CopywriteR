...
\title{\bf ENCODER: DNA copy number detection from off-target sequence data.}
...
ENhanced COpy number Detection from Exome Reads (ENCODER). Unlike other
currently available tools, ENCODER is based on off-target reads. We
...
\section{ENCODER workflow}

ENCODER uses .bam files as an input. These files are processed in
...
The full analysis of copy number data with ENCODER includes three sequential
steps, using the \Rcode{preENCODER()}, \Rcode{ENCODER()} and \Rcode{plotCNA()}
functions respectively.

In this analysis workflow, we will use ENCODER to extract copy number
...
\subsection{\Rcode{preENCODER()}}

ENCODER uses binned read count data as a basis for copy number detection.
The \Rcode{preENCODER()} function allows to create all the necessary helper
...
folder, which is required to run ENCODER. If previously created helper files for
a specific bin size are available, the {\Rcode{preENCODER()} step can be
omitted.

For this example, ENCODER will be applied to a bin size of 1 Mb. In our example,
the .bam files are kept in a folder called "data". We run {\Rcode{preENCODER()}
to generate helper files as follows:

<<load.ENCODER, print=FALSE, eval=TRUE>>=
library(ENCODER)
@
...
<<preENCODER, print=FALSE, eval=TRUE>>=
preENCODER(black.GC.mapa.folder = file.path(data.folder, "mm10_1kb"),
...
\subsection{\Rcode{ENCODER()}}

The \Rcode{ENCODER()} function allows to calculate binned read count data based
on the helper files created using \Rcode{preENCODER()}. ENCODER uses a peak
...
custom bin size helper files. ENCODER can use \Biocpkg{snowfall} for parallel
computing, where every sample is processed on a single core. The ncpu argument
defines the number of cores that will be maximally used by ENCODER during
...
ENCODER will create a new folder named CNAprofiles in the directory specified by
...
the last function of the ENCODER package, \Rcode{plotCNA()}. The log.txt file
...
I am interested in further improving ENCODER, so if ENCODER fails to analyze
