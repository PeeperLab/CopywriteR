plotCNA <- function(destinationFolder) {
	...
	for (i in 1:(ncol(read_count) - 4)) {
		...
		png(file=paste0(i, "_freqonly_", colnames_read_count[i],".png"), width=2*480, height=480)
			plot(nor[,i], dotres = 1)
		dev.off()	
		pdf(file=paste0(i, "_freqonly_", colnames_read_count[i],".pdf"), width=2*7, height=7)
			plot(nor[,i], dotres = 1)
		dev.off()
		png(file=paste0(i, "_", colnames_read_count[i],".png"), width=2*480, height=480)
			plot(calls[,i], dotres=1)
		dev.off()
		pdf(file=paste0(i, "_", colnames_read_count[i],".pdf"), width=2*7, height=7)
			plot(calls[,i], dotres=1)
		dev.off()
		png(file=paste0(i, "_segmented_", colnames_read_count[i],".png"), width=2*480, height=480)
			plot(seg[,i], dotres=1)
		dev.off()
		...
		for (j in 1:inputStructure$nchrom){
			...
			pdf(file=paste0("chromo_", j, "_", colnames_read_count[i],"_freqonly.pdf"), width=2*7, height=7)
				plot(nor[chromosomes(nor) == j,i], dotres=1)
			dev.off()
			png(file=paste0("chromo_", j, "_", colnames_read_count[i],"_seg.png"), width=2*480, height=480)
				plot(seg[chromosomes(seg) == j,i], dotres=1)
			dev.off()
			pdf(file=paste0("chromo_", j, "_", colnames_read_count[i],"_seg.pdf"), width=2*7, height=7)
				plot(seg[chromosomes(seg) == j,i], dotres=1)
			dev.off()
			png(file=paste0("chromo_", j, "_", colnames_read_count[i],".png"), width=2*480, height=480)
